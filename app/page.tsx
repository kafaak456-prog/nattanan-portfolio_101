  ...
  return (
    <main className="min-h-screen bg-[#0f1729] text-zinc-200 font-mono selection:bg-blue-400/30 overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-20 p-6 flex justify-between items-center max-w-7xl mx-auto border-b border-blue-900/40 backdrop-blur-md bg-[#0f1729]/80">
        ...
        <div className="text-[10px] text-blue-500/70 animate-pulse hidden sm:block">SYSTEM_STATUS: OPTIMAL</div>
      </nav>
        ...
        <section id="home" className="scroll-mt-32">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <div className="lg:col-span-7 space-y-10">
              <div>
                <div className="text-blue-400 text-xs mb-8 break-words">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <button
                  ...
                  className="px-8 sm:px-10 py-4 border border-blue-900 hover:border-blue-400 text-blue-400 font-bold uppercase transition-all backdrop-blur-sm text-center text-sm sm:text-base"
                >
                  Contact
                </button>
              </div>
          ...
          <div className="space-y-3">
            <a href="mailto:weerapongn27@vmi.edu"
              ...
              <div className="min-w-0">
                ...
                <div className="text-sm text-zinc-100 group-hover:text-blue-300 transition-colors break-all">weerapongn27@vmi.edu</div>
              </div>
            </a>
            <a href="https://linkedin.com/in/nattanan-weerapong-b900a0275" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-4 p-5 border border-zinc-800 hover:border-blue-600/60 bg-zinc-950/50 transition-all group">
              <div className="text-blue-400 text-xl w-8 font-bold">in</div>
              <div className="min-w-0">
                <div className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">LinkedIn</div>
                <div className="text-sm text-zinc-100 group-hover:text-blue-300 transition-colors break-all">linkedin.com/in/nattanan-weerapong-b900a0275</div>
              </div>
            </a>
            <a href="https://github.com/kafaak456-prog" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-4 p-5 border border-zinc-800 hover:border-blue-600/60 bg-zinc-950/50 transition-all group">
              <div className="text-blue-400 text-xl w-8 font-bold">{'{ }'}</div>
              <div className="min-w-0">
                <div className="text-[10px] text-zinc-500 uppercase tracking-widest mb-1">GitHub</div>
                <div className="text-sm text-zinc-100 group-hover:text-blue-300 transition-colors break-all">github.com/kafaak456-prog</div>
              </div>
            </a>
...
function InfoRow({ k, v, highlight }: { k: string; v: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between items-center py-2 border-b border-zinc-800/60 last:border-0 text-sm gap-3">
      <span className="text-zinc-500 shrink-0">{k}</span>
      <span className={`text-right ${highlight ? "text-blue-400 font-bold" : "text-zinc-200"}`}>{v}</span>
    </div>
  );
}
...
function SkillColumn({ label, items, accent }: { label: string; items: string[]; accent?: boolean }) {
  return (
    <div className="p-5 border border-zinc-800 bg-zinc-950/50 hover:border-blue-800/50 transition-colors">
      <div className={`text-[10px] uppercase tracking-widest mb-4 ${accent ? "text-blue-400" : "text-blue-500/70"}`}>{label}</div>
      <ul className="space-y-2">
        {items.map((item) => (
          <li key={item} className="flex items-center gap-3 text-sm text-zinc-200">
            <span className={`w-1.5 h-1.5 ${accent ? "bg-blue-400" : "bg-blue-700"}`} />
            <span>{item}</span>
          </li>
        ))}
      </ul>
